...
struct t8_data_per_element_waveletfree_3d_gh {
    ...
    double u_coeff_d3[M_mra]; // For dimension 3
    bool significant; // Whether an element is significant or not
    double d_coeff_wavelet_free_d1[M_mra][4]; // Extra coefficients for wavelet-free difference information for dimension 1
...
public:
    ...
    // Modifiers
    void insert(unsigned int level, uint64_t key, const T &data);
    void erase(unsigned int level, uint64_t key);
    void erase(unsigned int level);
    void erase_all();  // Erase all data at all levels

    // Iterators
    iterator begin(unsigned int level);
    iterator end(unsigned int level);
    const_iterator begin(unsigned int level) const;
    const_iterator end(unsigned int level) const;

    // Search
    std::optional<T> find(unsigned int level, uint64_t key) const;
    bool contains(unsigned int level, uint64_t key) const;

    // Size
    size_t size() const noexcept;

    // Operators
    map& operator[](unsigned int level);
    const map& operator[](unsigned int level) const;
    // Custom method to access data by both level and key
    T& get(unsigned int level, uint64_t key);  // Access data at specific level and key
    const T& get(unsigned int level, uint64_t key) const;  // Access data at specific level and key

private:
    void check_level(unsigned int level) const;
};
