...
//The polynomials are used to verify the Vanishing Moments
double zero_degree_large_const(double x, double y) {
  return 100000000000;
}
...
double complex_flow(double x, double y) {
...
double fractal_field(double x, double y) {
...
double testtest(double x, double y) {
...
double super_complex_function(double x, double y) {
    // ------------------------------------------------------
...
float calculate_correlation(float *array1, float *array2, int length) {
...
void find_indices(double xmin, double xmax, double ymin, double ymax,
                  int** x_indices, int** y_indices, int* count) {
...
void create_combinations_and_coordinates(int* x_indices, int* y_indices, int count,
                                          double* xa, double* ya,
...
void print_matching_points(int* is_inside, double* coordinates, int num_points) {
...
void fill_zvalues_arr_for_Correlation_wf_spline(t8_forest_t forest,float fileData_wf[721801],double* xa, double* ya) {
    ...
    for (ielement = 0; ielement < num_elements_in_tree; ++ielement, ++current_index) {
      ...
      for (int i = 0; i < count; ++i) {
          if (is_inside_arr[i] == 1) {
              // Each point has 3 values: x, y, z (so we access coordinates[3*i], coordinates[3*i+1], coordinates[3*i+2])
              fileData_wf[index_combinations[2*i]*601+(600-index_combinations[2*i+1])]=(float)AuswertungSinglescale_wf_spline(forest,scheme,coordinates[3 * i],coordinates[3 * i + 1],itree,ielement,current_index);
              //printf("[%.2f, %.2f, %.2f]\n", coordinates[3 * i], coordinates[3 * i + 1], coordinates[3 * i + 2]);
          }
      }
      free(x_indices);
...
void fill_zvalues_arr_for_Correlation_wb_spline(t8_forest_t forest,float fileData_wb[721801],double* xa, double* ya) {
  ...
  for (itree = 0, current_index = 0; itree < num_local_trees; ++itree) {
    ...
    for (ielement = 0; ielement < num_elements_in_tree; ++ielement, ++current_index) {
      ...
      for (int i = 0; i < count; ++i) {
          if (is_inside_arr[i] == 1) {
              // Each point has 3 values: x, y, z (so we access coordinates[3*i], coordinates[3*i+1], coordinates[3*i+2])
              fileData_wb[index_combinations[2*i]*601+(600-index_combinations[2*i+1])]=(float)AuswertungSinglescale_wb_spline(forest,scheme,coordinates[3 * i],coordinates[3 * i + 1],itree,ielement,current_index);
              //printf("[%.2f, %.2f, %.2f]\n", coordinates[3 * i], coordinates[3 * i + 1], coordinates[3 * i + 2]);
          }
      }
      free(x_indices);
...
int main(int argc, char** argv) {
  ...
  cout << "File data is\n";
  int max_level=7;
  levelgrid_map<t8_data_per_element_waveletfree_1d_gh>* grid = new levelgrid_map<t8_data_per_element_waveletfree_1d_gh>(max_level);
